...
class Snake():
    ...
    def getHeadPos(self): #Annab tagasi peapositsiooni väärtuse.
...
class FoodSpawner():
    ...
    def spawnFood(self):
        if self.isFoodOnScreen == False:
            self.position = [random.randrange(1, 50) * 10, random.randrange(1, 50) * 10]
            self.toitEkraanil = True
        return self.position
    
    def  setFoodOnScreen(self, b):
        isend.isFoodOnScreen = b
...
while True:
    ...
    if(snake.checkCollision() == 1):
        gameOver()
    pygame.display.set_caption("Ussimäng | Skoor :" + str(score))
    pygame.display.flip()
    fps.tick(24)